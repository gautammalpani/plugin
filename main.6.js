...
// Sisense plugins typically expose Angular on the page.
let mod;
try {
...
prism.registerWidget('filterBar', {
  ...
  data: {
    ...
    buildQuery: (widget, query) => {
      ...
      if (first) {
        const clone = $$.object.clone(first, true);
  ...
  render: (widget, args) => {
    ...
    const datasourceTitle =
      widget.dashboard?.datasource?.title ||
      widget.dashboard?.datasource ||
      null;
    ...
    const parseJaqlResponseToValues = (json) => {
      ...
      if (Array.isArray(json.data)) {
        return json.data.map(r => (Array.isArray(r) ? (r[0]?.text ?? r[0]?.data ?? r[0]) : r)).filter(Boolean);
      }
    ...
    const serverSearchMembers = async ({ dim, datatype, level, term, limit }) => {
      // Build a JAQL query for matching members.
      // Text: contains term; Number: equals if numeric; Datetime: contains (string fallback).
      let filter;
      if (datatype === 'text') {
        filter = { contains: term };
      } else if (datatype === 'number') {
        const n = Number(term);
        filter = isNaN(n) ? { contains: term } : { equals: n };
      } else if (datatype === 'datetime') {
        // Fallback for typeahead: treat as text match on formatted value.
        filter = { contains: term };
      } else {
        filter = { contains: term };
      }
    ...
    const renderListControl = ({ label, dim, datatype, level }) => {
      ...
      const doSearch = debounce(async () => {
        ...
        if (widget.style.serverTypeahead && datasourceTitle) {
          const values = await serverSearchMembers({ dim, datatype, level, term, limit: Number(widget.style.maxResults || 200) });
          addOptions(values);
        } else {
